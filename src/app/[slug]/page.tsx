  ...
  const wixClient = await wixClientServer();
    ...
    return (
      <div className="px-4 md:px:8 lg:px-16 xl:px-32 2xl:px-64 relative flex flex-col lg:flex-row gap-16">
        ...
        <div className="h-[2px] bg-gray-100 mt-10"></div>

        {product.price?.price === product.price?.discountedPrice ? (<h2 className="font-medium 2xl">{product.price?.price}</h2>) : (
          <div className="flex items-center gap-4">
            <h3 className="text-xl text-gray-500 line-through">{product.price?.price}</h3>
            <h2 className="font-medium text-3xl">{product.price?.discountedPrice}</h2>
          </div>
        )}

        <div className="h-[2px] bg-gray-100 mt-10" />
        ...
        {product.additionalInfoSections?.map((section: any) => (
          <div className="text-sm" key={section.title}>
            <h4 className="text-medium font-medium mb-4">{section.title}</h4>
